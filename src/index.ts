...
//     if (!environment) {
//       return res.status(404).json({ error: "Environment not found in Redis" });
//     }
...
//     } catch (err) {
//       return res.status(400).json({ error: "Invalid JSON in environment" });
//     }

//     const result = await PhaseExecutor.run(parsedEnv);
//     redisClient.set(`env:${workflowId}`, result.toString());
//     return res.status(200).json({ msg: "Execution done" });
//   }
// );
